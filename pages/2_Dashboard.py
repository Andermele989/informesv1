...
/* ---- HEADER ---- */
.dash-header-premium {
    background: linear-gradient(135deg, rgba(20,241,178,0.07) 0%, rgba(56,189,248,0.04) 60%, rgba(167,139,250,0.04) 100%);
    border: 1px solid rgba(20,241,178,0.12);
    border-radius: 20px;
...
.dash-header-premium::before {
    ...
    height: 2px;
    background: linear-gradient(90deg, transparent, #14f1b2 30%, #38bdf8 70%, transparent);
}
.dash-header-premium::after {
    content: '';
    position: absolute;
    bottom: -60px; right: -60px;
    width: 180px; height: 180px;
    background: radial-gradient(circle, rgba(20,241,178,0.06), transparent 70%);
    border-radius: 50%;
    pointer-events: none;
}
.dash-header-premium h1 {
    font-size: 2rem;
    font-weight: 800;
    background: linear-gradient(135deg, #14f1b2 0%, #38bdf8 60%, #a78bfa 100%);
...
.dash-meta-badge span { color: #14f1b2; font-weight: 600; }
...
.section-title-dash::after {
    ...
    flex: 1;
    height: 1px;
    background: linear-gradient(90deg, rgba(20,241,178,0.25), transparent);
}
...
.export-card:hover { border-color: rgba(20,241,178,0.2); }
...
div[data-testid="stButton"] > button[kind="primary"] {
    ...
    box-shadow: 0 4px 20px rgba(20,241,178,0.2) !important;
    transition: all 0.3s !important;
}
div[data-testid="stButton"] > button[kind="primary"]:hover {
    transform: translateY(-2px) !important;
    box-shadow: 0 8px 30px rgba(20,241,178,0.35) !important;
}
div[data-testid="stDownloadButton"] > button {
    background: linear-gradient(135deg, rgba(20,241,178,0.1), rgba(56,189,248,0.08)) !important;
    border: 1px solid rgba(20,241,178,0.25) !important;
    color: #14f1b2 !important;
    font-weight: 600 !important;
    border-radius: 12px !important;
    transition: all 0.3s !important;
}
div[data-testid="stDownloadButton"] > button:hover {
    background: linear-gradient(135deg, rgba(20,241,178,0.18), rgba(56,189,248,0.14)) !important;
    border-color: rgba(20,241,178,0.45) !important;
...
hr { border-color: rgba(148,163,184,0.08) !important; margin: 28px 0 !important; }

@media (max-width: 640px) {
    .dash-header-premium h1 { font-size: 1.4rem; }
...
def aplicar_layout_premium(fig, height=380, margin=None, mobile=False):
    ...
    fig.update_xaxes(showgrid=False, tickfont=dict(size=10 if mobile else 12, color="#cbd5e1", family="Outfit"), zeroline=False)
    fig.update_yaxes(showgrid=True, gridcolor="rgba(148,163,184,0.10)", tickfont=dict(size=10 if mobile else 12, color="#94a3b8", family="Outfit"), zeroline=False)
    return fig
...
# ========================= FILTROS SIDEBAR =========================
st.sidebar.markdown("""
<div style="padding: 4px 0 12px; border-bottom: 1px solid rgba(148,163,184,0.1); margin-bottom: 16px;">
    <div style="font-size:0.7rem; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; color:#14f1b2; margin-bottom:4px;">FILTROS</div>
</div>
""", unsafe_allow_html=True)
...
año_servicio_sel = st.sidebar.selectbox("📅 Año de Servicio", ["Todos"] + años_servicio_disp, index=default_index_año + 1)
...
meses_seleccionados = st.sidebar.multiselect(
    "📆 Mes(es)", meses_disponibles,
    default=[mes_actual] if mes_actual in meses_disponibles else ([meses_disponibles[0]] if meses_disponibles else [])
)

df_filtrado_mes = df_filtrado_año[df_filtrado_año["Mes"].isin(meses_seleccionados)] if meses_seleccionados else df_filtrado_año

st.sidebar.divider()
grupos_disponibles = sorted(df_filtrado_mes["Grupo"].unique().tolist())
grupo_sel = st.sidebar.selectbox("🏘️ Grupo", ["Todos"] + grupos_disponibles, index=0)
df_filtrado_grupo = df_filtrado_mes[df_filtrado_mes["Grupo"] == grupo_sel] if grupo_sel != "Todos" else df_filtrado_mes

publicadores_disponibles = sorted(df_filtrado_grupo["Publicador"].unique().tolist())
publicadores_sel = st.sidebar.multiselect("👤 Publicador", publicadores_disponibles, default=[], placeholder="Todos")
df_filtrado = df_filtrado_grupo[df_filtrado_grupo["Publicador"].isin(publicadores_sel)] if publicadores_sel else df_filtrado_grupo

orden_publicadores = [
    "Delis Requena", "Esteban Requena", "David Requena", "Erosilda Arbelaez",
    "Anderson Melendez (Aux.)", "Lina Sanchez", "Teonilda Porto", "Diana Maestre",
    "Yenfer Ojeda (S. de grupo)", "Yolmarys Ojeda", "Mario Arbelaez",
    "Zenaida Jimenez", "Ender Requena", "Jose Arbelaez", "Elizandrit Rebolledo"
]

if not df_filtrado.empty:
    def get_sort_index(val):
        try: return orden_publicadores.index(val)
        except ValueError: return len(orden_publicadores)
...
if grupo_sel != "Todos": filtro_label += f" · {grupo_sel}"
if publicadores_sel: filtro_label += f" · {len(publicadores_sel)} pub."
...
delta_inf = delta_hrs = delta_cur = ""
total_inf_ant = total_hrs_ant = total_cur_ant = 0
mes_ant_str = None
...
if len(meses_seleccionados) == 1:
    ...
    except: mes_ant_str = None

    if mes_ant_str:
        df_mes_ant = df[df["Mes"] == mes_ant_str]
        if grupo_sel != "Todos": df_mes_ant = df_mes_ant[df_mes_ant["Grupo"] == grupo_sel]
        if publicadores_sel: df_mes_ant = df_mes_ant[df_mes_ant["Publicador"].isin(publicadores_sel)]
...
<div class="dash-header-premium">
    ...
    <div class="dash-header-meta">
        <div class="dash-meta-badge">📅 <span>{etiqueta_periodo}</span></div>
        <div class="dash-meta-badge">🏘️ <span>{grupo_sel}</span></div>
        <div class="dash-meta-badge">📄 <span>{total_informes_mes}</span> informes</div>
        <div class="dash-meta-badge">🕒 <span>{total_horas_mes}</span> horas</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
def render_card(label, value, icon, icon_class, footer, delta=""):
    glow_class = icon_class.replace("icon-", "glow-")
    return f"""<div class="metric-card {glow_class}">
...
# ========================= TABLA =========================
st.divider()
st.markdown('<div class="section-title-dash">📊 Datos del Periodo</div>', unsafe_allow_html=True)
...
# ========================= GRÁFICO 1 =========================
st.divider()
st.markdown('<div class="section-title-dash">🕒 Horas del Mes por Publicador</div>', unsafe_allow_html=True)

df_graficos = df_filtrado[df_filtrado["Privilegios"] != "Inactivo"].copy()
if not df_graficos.empty:
    df_horas_pub = df_graficos.groupby("Publicador", as_index=False)["Horas"].sum().sort_values("Horas", ascending=False)
    ...
    for i in range(n_bars):
        ...
        bar_colors.append(f"rgb({int(ratio*16)},{int(200-ratio*55)},{int(150-ratio*21)})")

    fig1 = go.Figure()
    fig1.add_trace(go.Bar(
        x=df_horas_pub["Publicador_Corto"], y=df_horas_pub["Horas"],
        text=df_horas_pub["Horas"].apply(lambda x: f"<b>{x}</b>"),
        textposition="outside", textfont=dict(size=14, color="#00e8a2", family="Outfit"),
    ...
    fig1.update_layout(
        ...
        font=dict(family="Outfit", size=13, color="#ffffff"), showlegend=False,
        xaxis=dict(showgrid=False, title="", tickfont=dict(size=11, color="#c8d6e5", family="Outfit"),
                   tickangle=-25 if n_bars > 6 else 0, categoryorder="total descending"),
        yaxis=dict(showgrid=True, gridcolor="rgba(255,255,255,0.04)", title="",
                   range=[0, max_h_pub * 1.22], tickfont=dict(size=11, color="#8899aa", family="Outfit"), zeroline=False),
        margin=dict(t=25, b=60, l=45, r=25), height=380, bargap=0.25,
...
# ========================= GRÁFICO 2 =========================
st.divider()
st.markdown('<div class="section-title-dash">📈 Evolución de Horas del Grupo por Mes</div>', unsafe_allow_html=True)

df_evolucion_base = df.copy()
if grupo_sel != "Todos": df_evolucion_base = df_evolucion_base[df_evolucion_base["Grupo"] == grupo_sel]
if publicadores_sel: df_evolucion_base = df_evolucion_base[df_evolucion_base["Publicador"].isin(publicadores_sel)]

df_evolucion = df_evolucion_base[df_evolucion_base["Privilegios"] != "Inactivo"].copy()
if not df_evolucion.empty:
    df_mensual = df_evolucion.groupby("Mes", as_index=False)["Horas"].sum().sort_values("Mes")
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=df_mensual["Mes"], y=df_mensual["Horas"], mode='lines',
        line=dict(color="rgba(59,130,246,0)", width=0), fill='tozeroy',
        fillcolor='rgba(59,130,246,0.07)', showlegend=False, hoverinfo='skip'))
    fig2.add_trace(go.Scatter(x=df_mensual["Mes"], y=df_mensual["Horas"], mode='lines',
        line=dict(color="rgba(96,165,250,0.3)", width=12, shape='spline'), showlegend=False, hoverinfo='skip'))
    fig2.add_trace(go.Scatter(
        x=df_mensual["Mes"], y=df_mensual["Horas"], mode='lines+markers+text',
        text=df_mensual["Horas"].apply(lambda x: f"<b>{x}</b>"), textposition="top center",
        textfont=dict(size=16, color="#93c5fd", family="Outfit"),
        line=dict(color="#3b82f6", width=3.5, shape='spline'),
        marker=dict(size=12, color="#3b82f6", line=dict(width=3, color="#0f111a"), symbol="circle"),
        name="Horas", hovertemplate="<b>%{x}</b><br>Horas: %{y}<extra></extra>"
    ))
    ...
    fig2.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor="rgba(255,255,255,0.04)", title="",
                   range=[0, max_h_ev * 1.25], tickfont=dict(size=11, color="#8899aa", family="Outfit"), zeroline=False),
        margin=dict(t=30, b=40, l=45, r=25), height=340, showlegend=False,
...
with col_t1:
    st.markdown('<div class="section-title-dash">🏆 Desempeño de Precursores</div>', unsafe_allow_html=True)
with col_t2:
    st.markdown('<div style="height: 35px;"></div>', unsafe_allow_html=True)
    tipo_prec = st.selectbox("Tipo de precursor",
        options=["Todos", "Precursores Regulares", "Precursores Auxiliares"], index=0, label_visibility="collapsed")
...
if not df_precursores.empty:
    df_prec_resumen = df_precursores.groupby("Publicador", as_index=False).agg({"Horas": "sum", "Cursos Bíblicos": "sum"})
    ...
    prec_colors = [f"rgb({int(139-i/max(n_prec-1,1)*48)},{int(92-i/max(n_prec-1,1)*39)},{int(246-i/max(n_prec-1,1)*64)})" for i in range(n_prec)]

    fig3 = go.Figure()
    fig3.add_trace(go.Bar(
        y=df_prec_resumen["Publicador_Corto"], x=df_prec_resumen["Horas"], orientation='h',
        marker=dict(color=prec_colors, line=dict(width=0), opacity=0.9),
        text=df_prec_resumen["Horas"].apply(lambda x: f"<b>{x}h</b>"),
        textposition="outside", textfont=dict(size=13, color="#c4b5fd", family="Outfit"),
        customdata=df_prec_resumen["Publicador"], name="Horas",
    ...
    fig3.update_layout(
        ...
        title=dict(text="<b>Horas de Precursores</b>", font=dict(color="#ffffff", size=16, family="Outfit"), x=0.5, xanchor="center"),
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font=dict(family="Outfit", color="white", size=13),
        xaxis=dict(title="", tickfont=dict(color="#8899aa", size=11, family="Outfit"),
                   gridcolor="rgba(255,255,255,0.04)", zerolinecolor="rgba(255,255,255,0.04)", range=[0, max_h_prec * 1.22]),
        yaxis=dict(tickfont=dict(color="#c8d6e5", size=11, family="Outfit"),
                   gridcolor="rgba(255,255,255,0.04)", zerolinecolor="rgba(255,255,255,0.04)"),
        showlegend=False, margin=dict(t=45, b=25, l=85, r=45), bargap=0.2,
        transition=dict(duration=800, easing="cubic-in-out")
    )
    ...
    with col_p1: st.metric("Total Horas Precursores", f"{df_prec_resumen['Horas'].sum()} hrs")
    with col_p2: st.metric("Total Cursos Bíblicos", df_prec_resumen["Cursos Bíblicos"].sum())
else:
    st.info("No hay datos de precursores en este periodo.")

# ========================= GRÁFICO 4 =========================
st.divider()
st.markdown('<div class="section-title-dash">🍩 Distribución de Horas por Publicador</div>', unsafe_allow_html=True)
...
if not df_circular.empty:
    ...
    fig_pie.add_trace(go.Pie(
        labels=df_circular["Publicador_Corto"], values=df_circular["Horas"], hole=0.62,
        marker=dict(colors=PREMIUM_COLORS[:len(df_circular)], line=dict(color='#0f111a', width=2.5)),
        customdata=df_circular["Publicador"], textposition='outside', textinfo='percent+label',
        textfont=dict(size=11, color="#c8d6e5", family="Outfit"),
        hovertemplate="<b>%{customdata}</b><br>Horas: %{value}<br>%{percent}<extra></extra>",
        pull=[0.03] * len(df_circular), rotation=90
    ))
    fig_pie.update_layout(
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font=dict(family="Outfit", size=12, color="#ffffff"), showlegend=False,
        margin=dict(t=35, b=35, l=40, r=40), height=380,
        annotations=[dict(
            text=f"<b>{total_hrs_pie}</b><br><span style='font-size:11px;color:#8899aa'>Horas<br>Totales</span>",
            x=0.5, y=0.5, font=dict(size=22, color="#00c896", family="Outfit"), showarrow=False
        )],
...
st.divider()
st.markdown('<div class="section-title-dash">📥 Exportar Reporte</div>', unsafe_allow_html=True)
...
with st.container():
    ...
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        ...
        borde_grueso_ext = Border(left=Side(style='medium', color='1E3A5F'), right=Side(style='medium', color='1E3A5F'),
                                   top=Side(style='medium', color='1E3A5F'), bottom=Side(style='medium', color='1E3A5F'))
        borde_fino = Border(left=Side(style='thin', color='B0C4D8'), right=Side(style='thin', color='B0C4D8'),
                             top=Side(style='thin', color='B0C4D8'), bottom=Side(style='thin', color='B0C4D8'))
        ...
        for col_idx in range(1, num_cols + 1):
            worksheet.column_dimensions[worksheet.cell(row=2, column=col_idx).column_letter].width = 22
        ...
        for r_idx in range(header_row, data_start + num_filas):
            ...
            for c_idx in range(1, num_cols + 1):
                ...
                if r_idx == header_row:
                    ...
                    cell.font = font_data; cell.border = borde_fino
                    cell.alignment = Alignment(horizontal='left' if c_idx == 4 else 'center', vertical='center', wrap_text=True)
    ...
    with col_ex1:
        ...
        <div style="padding: 4px 0;">
            <div style="font-size:0.85rem; color:#94a3b8; margin-bottom:4px;">Archivo listo para descargar</div>
            <div style="font-size:0.78rem; color:#475569; font-family:'JetBrains Mono',monospace;">{excel_filename}</div>
        </div>
        """, unsafe_allow_html=True)
    with col_ex2:
        st.download_button(
...
# ========================= ANÁLISIS IA =========================
st.divider()
st.markdown('<div class="section-title-dash">🤖 Análisis Inteligente con IA</div>', unsafe_allow_html=True)
...
with col_ia_1:
    ...
    st.caption(f"Modelo: `{modelo_visible}`")
    btn_ia = st.button("✨ Generar Reporte con IA", use_container_width=True, type="primary")
with col_ia_2:
    st.markdown("""
    <div style="padding: 12px 0; color: #64748b; font-size: 0.85rem; line-height: 1.6;">
        Genera un análisis detallado del periodo seleccionado usando inteligencia artificial.
        Incluye comparación con el mes anterior, logros del grupo y sugerencias de mejora.
        El resultado también se exporta como <strong style="color:#94a3b8">PDF descargable</strong>.
    </div>
    """, unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
if btn_ia:
    with st.status("🧠 Analizando datos y generando reporte...", expanded=True) as status:
        ...
        comparacion_disponible = mes_ant_str is not None and total_inf_ant > 0
        comparacion_text = ""
        if comparacion_disponible:
            comparacion_text = f"""
            COMPARACIÓN CON EL MES ANTERIOR ({mes_ant_str}):
            - Informes el mes pasado: {total_inf_ant} (Mes actual: {total_pubs})
            - Horas el mes pasado: {total_hrs_ant} (Mes actual: {total_horas_ia})
            - Cursos el mes pasado: {total_cur_ant} (Mes actual: {total_cursos_ia})
            """
        ...
        - Precursores: {prec_text if not precursores.empty else 'No hay precursores.'}
        {comparacion_text}
        
        Genera un informe estructurado con:
        1. RESUMEN EJECUTIVO
        2. COMPARACIÓN CON EL MES ANTERIOR
        3. PUNTOS FUERTES Y LOGROS
        4. ÁREAS DE MEJORA Y SUGERENCIAS
        
        Sé profesional, motivador y preciso. Utiliza Markdown limpio."""
        ...
        try:
            ...
            if ai_opcion == "OpenAI":
                ...
                if api_key:
                    ...
                    res = client.chat.completions.create(model=OPENAI_MODEL, messages=[{"role": "user", "content": prompt}], temperature=0.35)
            ...
            if resultado:
                ...
                img_paths = {}
                PDF_BG = "#0f172a"; PDF_PLOT_BG = "rgba(15,23,42,0.95)"
                PDF_GRID = "rgba(148,163,184,0.12)"; PDF_TEXT = "#e2e8f0"; PDF_MUTED = "#94a3b8"
                ...
                try:
                    ...
                    if not df_g1.empty:
                        n1 = len(df_g1)
                        colors1 = [f"rgb({int(i/max(n1-1,1)*16)},{int(220-i/max(n1-1,1)*60)},{int(160-i/max(n1-1,1)*25)})" for i in range(n1)]
                        fig1_pdf = go.Figure()
                        fig1_pdf.add_trace(go.Bar(x=df_g1["Publicador"], y=df_g1["Horas"],
                            text=df_g1["Horas"].apply(lambda x: f"<b>{x}</b>"), textposition="outside",
                            textfont=dict(size=18, color="#00e8a2", family="Arial Black"),
                            marker=dict(color=colors1, line=dict(width=0), opacity=0.92)))
                        fig1_pdf.update_layout(plot_bgcolor=PDF_PLOT_BG, paper_bgcolor=PDF_BG,
                            font=dict(family="Arial", size=14, color=PDF_TEXT), showlegend=False,
                            title=dict(text="<b>🕒 Horas del Mes por Publicador</b>",
                                font=dict(size=22, color="#ffffff", family="Arial Black"), x=0.5, xanchor="center", y=0.96),
                            xaxis=dict(showgrid=False, title="", tickfont=dict(size=13, color=PDF_TEXT, family="Arial"),
                                tickangle=-20 if n1 > 6 else 0, categoryorder="total descending"),
                            yaxis=dict(showgrid=True, gridcolor=PDF_GRID, title="", range=[0, df_g1["Horas"].max()*1.25],
                                tickfont=dict(size=13, color=PDF_MUTED, family="Arial"), zeroline=False),
                            margin=dict(t=60, b=80, l=55, r=30), bargap=0.25)
                        img_paths['fig1'] = _save_premium_fig(fig1_pdf, "fig1_pdf", w=950, h=440)
                except: pass

                try:
                    df_ev_pdf = df_evolucion_base[df_evolucion_base["Privilegios"] != "Inactivo"].copy()
                    if not df_ev_pdf.empty:
                        df_m_pdf = df_ev_pdf.groupby("Mes", as_index=False)["Horas"].sum().sort_values("Mes")
                        if not df_m_pdf.empty:
                            fig2_pdf = go.Figure()
                            fig2_pdf.add_trace(go.Scatter(x=df_m_pdf["Mes"], y=df_m_pdf["Horas"], mode='lines',
                                line=dict(color="rgba(59,130,246,0)", width=0), fill='tozeroy',
                                fillcolor='rgba(59,130,246,0.12)', showlegend=False, hoverinfo='skip'))
                            fig2_pdf.add_trace(go.Scatter(x=df_m_pdf["Mes"], y=df_m_pdf["Horas"], mode='lines',
                                line=dict(color="rgba(96,165,250,0.25)", width=14, shape='spline'), showlegend=False, hoverinfo='skip'))
                            fig2_pdf.add_trace(go.Scatter(x=df_m_pdf["Mes"], y=df_m_pdf["Horas"], mode='lines+markers+text',
                                text=df_m_pdf["Horas"].apply(lambda x: f"<b>{x}</b>"), textposition="top center",
                                textfont=dict(size=18, color="#93c5fd", family="Arial Black"),
                                line=dict(color="#3b82f6", width=4, shape='spline'),
                                marker=dict(size=14, color="#3b82f6", line=dict(width=3, color=PDF_BG), symbol="circle"), name="Horas"))
                            fig2_pdf.update_layout(plot_bgcolor=PDF_PLOT_BG, paper_bgcolor=PDF_BG,
                                font=dict(family="Arial", size=14, color=PDF_TEXT),
                                title=dict(text="<b>📈 Evolución de Horas del Grupo por Mes</b>",
                                    font=dict(size=22, color="#ffffff", family="Arial Black"), x=0.5, xanchor="center", y=0.96),
                                xaxis=dict(showgrid=False, title="", type="category",
                                    range=[-0.5, len(df_m_pdf)-0.5], tickfont=dict(size=14, color=PDF_TEXT, family="Arial")),
                                yaxis=dict(showgrid=True, gridcolor=PDF_GRID, title="", range=[0, df_m_pdf["Horas"].max()*1.3],
                                    tickfont=dict(size=13, color=PDF_MUTED, family="Arial"), zeroline=False),
                                margin=dict(t=60, b=55, l=60, r=30), showlegend=False)
                            img_paths['fig2'] = _save_premium_fig(fig2_pdf, "fig2_pdf", w=950, h=420)
                except: pass

                try:
                    if not df_precursores.empty:
                        df_prec_pdf = df_precursores.groupby("Publicador", as_index=False).agg({"Horas": "sum", "Cursos Bíblicos": "sum"}).sort_values("Horas", ascending=True)
                        n3 = len(df_prec_pdf)
                        colors3 = [f"rgb({int(139-i/max(n3-1,1)*48)},{int(92-i/max(n3-1,1)*39)},{int(246-i/max(n3-1,1)*64)})" for i in range(n3)]
                        fig3_pdf = go.Figure()
                        fig3_pdf.add_trace(go.Bar(y=df_prec_pdf["Publicador"], x=df_prec_pdf["Horas"], orientation='h',
                            marker=dict(color=colors3, line=dict(width=0), opacity=0.9),
                            text=df_prec_pdf["Horas"].apply(lambda x: f"<b>{x} hrs</b>"), textposition="outside",
                            textfont=dict(size=16, color="#c4b5fd", family="Arial Black"), name="Horas"))
                        fig3_pdf.update_layout(height=max(300, n3*60+120),
                            title=dict(text=f"<b>🏆 Desempeño de Precursores</b>",
                                font=dict(color="#ffffff", size=22, family="Arial Black"), x=0.5, xanchor="center", y=0.96),
                            plot_bgcolor=PDF_PLOT_BG, paper_bgcolor=PDF_BG, font=dict(family="Arial", color=PDF_TEXT, size=14),
                            xaxis=dict(title="", tickfont=dict(color=PDF_MUTED, size=13, family="Arial"),
                                gridcolor=PDF_GRID, zerolinecolor=PDF_GRID, range=[0, df_prec_pdf["Horas"].max()*1.25]),
                            yaxis=dict(tickfont=dict(color=PDF_TEXT, size=14, family="Arial"), gridcolor=PDF_GRID, zerolinecolor=PDF_GRID),
                            showlegend=False, margin=dict(t=60, b=30, l=140, r=70), bargap=0.3)
                        img_paths['fig3'] = _save_premium_fig(fig3_pdf, "fig3_pdf", w=950, h=max(300, n3*60+120))
                except: pass
                ...
                try:
                    ...
                    if not df_pie_pdf.empty:
                        ...
                        fig_pie_pdf.add_trace(go.Pie(labels=df_pie_pdf["Publicador"], values=df_pie_pdf["Horas"], hole=0.62,
                            marker=dict(colors=PREMIUM_COLORS[:len(df_pie_pdf)], line=dict(color=PDF_BG, width=3)),
                            textposition='outside', textinfo='percent+label',
                            textfont=dict(size=14, color=PDF_TEXT, family="Arial"),
                            pull=[0.03]*len(df_pie_pdf), rotation=90))
                        fig_pie_pdf.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor=PDF_BG,
                            font=dict(family="Arial", size=14, color=PDF_TEXT),
                            title=dict(text="<b>🍩 Distribución de Horas por Publicador</b>",
                                font=dict(size=22, color="#ffffff", family="Arial Black"), x=0.5, xanchor="center", y=0.97),
                            showlegend=False, margin=dict(t=65, b=40, l=90, r=90),
                            annotations=[dict(text=f"<b>{total_pie}</b><br><span style='font-size:14px;color:#94a3b8'>Horas<br>Totales</span>",
                                x=0.5, y=0.5, font=dict(size=32, color="#00c896", family="Arial Black"), showarrow=False)])
                ...
                pdf.draw_section_header("1. TABLA COMPARATIVA DE METRICAS CLAVE")
                pdf.set_fill_color(30, 41, 59)
                pdf.set_text_color(255, 255, 255)
                pdf.set_font('Helvetica', 'B', 9)
                col_widths = [62, 38, 38, 42]
                for i, (h, w) in enumerate(zip([" Metrica", "Mes Anterior", "Mes Actual", "Variacion"], col_widths)):
                ...
                metrics_list = [("Publicadores Activos", total_inf_ant, total_pubs),
                                ("Horas de Predicacion", total_hrs_ant, total_horas_ia),
                                ("Cursos Biblicos", total_cur_ant, total_cursos_ia)]
                pdf.set_font('Helvetica', '', 9)
                for idx, (name, ant, act) in enumerate(metrics_list):
                    pdf.set_fill_color(241, 245, 249 if idx % 2 == 0 else 255)
                    ...
                    pdf.set_text_color(16, 185, 129 if pct_str.startswith('+') else (239, 68, 68) if pct_str.startswith('-') else 71)
                    pdf.set_font('Helvetica', 'B', 9)
                ...
                pdf.draw_section_header("2. ANALISIS DETALLADO DE LA IA")
                pdf.set_font('Helvetica', '', 9)
                pdf.set_text_color(30, 41, 59)
                for line in resultado.split('\n'):
                    line = line.strip()
                    if not line: pdf.ln(1.5); continue
                    if line.startswith('###') or (line.startswith('**') and line.endswith('**')) or line.upper().startswith(('1.', '2.', '3.', '4.')):
                        pdf.ln(2.5)
                        pdf.set_font('Helvetica', 'B', 10)
                        pdf.set_text_color(15, 23, 42)
                        pdf.multi_cell(0, 5, pdf.to_latin1(line.replace('###', '').replace('**', '').strip()), new_x="LMARGIN", new_y="NEXT")
                        pdf.set_font('Helvetica', '', 9)
                        pdf.set_text_color(30, 41, 59)
                    elif line.startswith('-') or line.startswith('*'):
                        pdf.multi_cell(0, 4.5, pdf.to_latin1(f"  · {line.lstrip('-*').strip()}"), new_x="LMARGIN", new_y="NEXT")
                    else:
                        pdf.multi_cell(0, 4.5, pdf.to_latin1(line), new_x="LMARGIN", new_y="NEXT")
                pdf.ln(4)

                charts_added = False
                for key, title_lbl in [("fig1","Horas por Publicador"),("fig2","Evolución de Horas"),("fig3","Precursores"),("fig_pie","Distribución")]:
                    path = img_paths.get(key)
                    if path and os.path.exists(path):
                        if not charts_added:
                            pdf.add_page()
                            pdf.draw_section_header("3. ANEXO: GRAFICOS DE RENDIMIENTO")
                            charts_added = True
                        elif pdf.get_y() > 145:
                            pdf.add_page()
                        pdf.set_font('Helvetica', 'B', 10)
                ...
                with open(pdf_path, "rb") as f:
                    pdf_bytes = f.read()
                st.download_button(label="📥 Descargar PDF del Informe", data=pdf_bytes,
                    file_name=pdf_filename, mime="application/pdf", type="primary")
